...
function jobSource(executionContext) {
    ...
    if (formContext.getAttribute("dia_vessel").getValue() != null) {
        ...
        req.open("GET", Xrm.Page.context.getClientUrl() + "/api/data/v9.1/dia_vessels(" + vesselId.replace("{", "").replace("}", "").toLowerCase() + ")?$select=_dia_batch_value, dia_occupation", false);
